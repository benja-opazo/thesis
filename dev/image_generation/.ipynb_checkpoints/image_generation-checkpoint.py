...
def smilextract_data_mean(data, measure, datalabel):
	data_list = []
...
def data_image(data, name, title, xlabel, ylabel):
	...
	ax.legend(['Masculine', 'Femenine 1','Femenine 2'], bbox_to_anchor=(0, -0.4, 1, 0), loc="lower left", mode="expand", ncol=3)
...
objective_measure = [["cpp", "CPP"], ["ps", "Spectral Slope"], ["pesq", "PESQ"], ["hnr", "Harmonic to Noise Ratio"], ["jitter_and_shimmer", "Jitter", "Shimmer"]]
...
for ind_var in independent_variables:
	for measure in objective_measure:
		if (measure[0] == "hnr" or measure[0]=="jitter_and_shimmer"):
			continue
		input_data = []
		mean_input_data =[]
		...
		for sounds in sound_names:
			print(file_prefix + ind_var[0] + "_" + sounds + "_" + measure[0] + ".csv")
