...
def data_image(data, name, title, xlabel, ylabel, legend, isthick=True):
    ...
    for dataplot in data:
        ...
        if isthick:        
            ...
            plt.plot(dataplot.index, dataplot[dataplot.columns[0]], linewidth=0.8)
...
synth_spectrum_modal = pd.read_csv("../outputs/figures/y_modal_fft.csv",index_col=0)
synth_spectrum_breathy = pd.read_csv("../outputs/figures/y_breathy_fft.csv",index_col=0)
input_spectrum = pd.read_csv("../outputs/figures/x_fft.csv",index_col=0)
...
data_image([synth_spectrum_modal], "synth_spectrum_modal", "Spectrum of modal Synthesized signal", "Frequency [Hz]", "Amplitude [dB]",["Spectrum"], isthick=False)
data_image([synth_spectrum_breathy], "synth_spectrum_breathy", "Spectrum of breathy Synthesized signal", "Frequency [Hz]", "Amplitude [dB]",["Spectrum"], isthick=False)
data_image([input_spectrum], "input_spectrum", "Spectrum of Input signal", "Frequency [Hz]", "Amplitude [dB]",["Spectrum"], isthick=False)